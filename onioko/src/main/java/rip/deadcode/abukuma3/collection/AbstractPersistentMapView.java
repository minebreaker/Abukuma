...
public abstract class AbstractPersistentMapView<K, V, T extends PersistentMapView<K, V, T>>
        extends ForwardingMap<K, V> implements PersistentMapView<K, V, T> {

    private final ImMap<K, V> delegate;

    protected AbstractPersistentMapView() {
        this.delegate = PersistentHashMap.empty();
    }

    protected AbstractPersistentMapView( Envelope<K, V> delegate ) {
